...
def format_from_binary(data, init=True):
    # converts objects stored in MongoDB as binary strings back into python class instances
    for k, v in data.iteritems():
        if isinstance(v, dict):
            format_from_binary(v, init=False)
        else:
            #print "%s: %s" % (k, type(v))
...
def format_for_mongo_insert(data, init=True):

    # converts class instances into binary strings and converts numpy data types to core python for storage in MongoDB

    def is_instance_userdefined_and_newclass(inst):
        # returns True if is instance of any user-defined class
        cls = inst.__class__
    ...
    for k, v in data.iteritems():
        #print "%s: %s" % (k, type(v))
        if isinstance(v, dict):
            format_for_mongo_insert(v, init=False)

        else:

            if isinstance(v, numpy.ndarray):
                data[k] = v.tolist()
            elif is_instance_userdefined_and_newclass(v):
                data[k] = bson.binary.Binary(pickle.dumps( v, protocol=2))
            elif isinstance(v, numpy.float64):
                data[k] = float(v)
            elif isinstance(v, (numpy.int,numpy.int32)):
                data[k] = int(v)
            elif isinstance(v, datetime.date):
                data[k] = datetime.datetime(v.year,v.month,v.day,0,0)
                ...
                if isinstance(v[0], datetime.date):
                    ...
                    for i in v:
                        try:
                            simp_list.append(datetime.datetime(i.year,i.month,i.day,0,0))
                        except:
                            simp_list.append(i)
                    data[k] = simp_list

                if is_instance_userdefined_and_newclass(v[0]):
                    print "bottom"
                    print "%s : %s" % (k, v)
                    simp_list = []
                    for i in v:
                        print i
                        print type(i)
                        simp_list.append(bson.binary.Binary(pickle.dumps( i, protocol=2)))
                        """
                        try:
                            simp_list.append(bson.binary.Binary(pickle.dumps( i, protocol=2)))
                            print "successful"
...
class db(object):
    ...
    def __init__(self, host = "localhost", db = "steadyfa_sf_model", cursorclass = None):
        ...
        if socket.gethostname() == "a1.levelskies.net":
        ...
        elif socket.gethostname() == "Ryans-Vaio":
            # these credentials should work on Ryan's local machine
            self.user = "root"
            self.passwd = "bitnami" # consider removing this
            self.unix_socket = ''
        else:
            # these credentials should work on the api.levelskies.com server
            self.user = "root"
            self.passwd = "bitnami"
    ...
    def insert_data(self, table, values,):
        ...
        query = "INSERT INTO %s.%s" % (self.db,table)
        if len(row)>0:
            count = 1
            query += ' ( '
                ...
                if len(row)>0:
                    ...
                    for v in row.itervalues():
                        ...
                        if count < len(row):
                            ...
                            row_query += " )"
                        count += 1
                    insert_bank.append(row_query)
            query += ', '.join(insert_bank)
    ...
    def join_fares(self, route, max_dist=None, flex=True, max_date_collected=None, min_date_collected=None):
        ...
        if not self.does_table_exist("%s_min_fares" % (route)):
            self.build_min_fare_table(route, max_date_collected=None, min_date_collected=None)
        ...
        query = ("CREATE TABLE %s_price_change_source "
                "SELECT a.price as beg_price, b.price as end_price, a.count, a.date_collected, b.date_collected as date_completed, a.trip_length, "
                ...
                "FROM %s_min_fares as a, %s_min_fares as b "
                "WHERE a.depart_length > b.depart_length "
                "AND (a.depart_date - b.depart_date) = 0 "
                "AND (a.return_date - b.return_date) = 0 "
                "ORDER BY a.date_collected DESC " % (route, route, route))

        query_ex = self.cur.execute(query)
        self.drop_existing('%s_min_fares' % (route))


    def pop_inputs(self, route, historical_windows, dates_only=False):
        ...
        """
        if dates_only:
            source = "%s_min_fares" % (route)
        else:
            source = "%s_price_change_source" % (route)
        # get unique search params
        unique_inputs = {}
        query = ("SELECT DISTINCT `date_collected`, `depart_date`, `day_tag_r`  FROM `%s` " % (source))
        query_ex = self.cur.execute(query)
        unique_inputs['price_depart_date'] = self.cur.fetchall()

        query = ("SELECT DISTINCT `date_collected`, `return_date`, `day_tag_d`  FROM `%s` " % (source))
        query_ex = self.cur.execute(query)
        unique_inputs['price_return_date'] = self.cur.fetchall()

        if not dates_only:
            query = ("SELECT DISTINCT `date_collected`, `depart_date`, `day_tag_r`, `proj_length`  FROM `%s` " % (source))
            query_ex = self.cur.execute(query)
            unique_inputs['inc_depart_date'] = self.cur.fetchall()

            query = ("SELECT DISTINCT `date_collected`, `return_date`, `day_tag_d`, `proj_length`  FROM `%s` " % (source))
            query_ex = self.cur.execute(query)
    ...
    def find_hist_analysis(self, route, historical_windows, unique_inputs=None, dates_only=False):
        ...
        if dates_only:
            source = "%s_min_fares" % (route)
            target = 'price'
            inputs = self.sel_crit('all', '%s' % (source), ['*'], {})
        else:
            source = "%s_price_change_source" % (route)
            target = 'end_price'
            inputs = self.sel_crit('all', '%s' % (source), ['*'], {})
        ...
        # filter by date
        full_length = len(inputs)
        for counter, inp in enumerate(inputs):
            row = dict(inp.items())
            row['date_completed'] = inp['date_collected'] + datetime.timedelta(days = inp['proj_length'])
            print "Out of %s: %s" % (full_length, counter)
            for index, i in enumerate(('depart','return',)):
                ...
                query = ("SELECT * "
                        ...
                        "AND `date_collected` <= %s " % (source, i, lower_date, i, upper_date, depart_weekday, return_weekday, inp['date_collected']))
                query_ex = self.cur.execute(query)
                ...
                if len(table) > 0:
                    ...
                    for index_2, j in enumerate(historical_windows):
                        ...
                        group = temp_rows[(
                                    ...
                                    )][:,(columns.index('%s' % (target)),columns.index('count'))]

                        if group[:,0].size > 0:
                            avg_met =  numpy.mean(group[:,0])
                            avg_count =  numpy.mean(group[:,1])
                        else:
                            avg_met = None
                            avg_count = None

                        if group[:,0].size > 1:
                            st_dev_met = numpy.std(group[:,0])
                            st_dev_count = numpy.std(group[:,1])
                        else:
                            st_dev_met = None
                            st_dev_count = None


                        row['%s_date_avg_fare_%s' % (i,j)] = avg_met
                        row['%s_date_st_dev_fare_%s' % (i,j)] = st_dev_met
                        #row['%s_date_avg_fare_%s_count' % (i,j)] = avg_count
                        #row['%s_date_st_dev_fare_%s_count' % (i,j)] = st_dev_count
                else:
                    for index_2, j in enumerate(historical_windows):
                        row['%s_date_avg_fare_%s' % (i,j)] = None
                        row['%s_date_st_dev_fare_%s' % (i,j)] = None
                        #row['%s_date_avg_fare_%s_count' % (i,j)] = None
                        #row['%s_date_st_dev_fare_%s_count' % (i,j)] = None
            ...
            if not dates_only:
                ...
                query = ("SELECT * "
                        ...
                        "AND (`day_tag_d` = %s AND `day_tag_r` = %s) "
                        "AND `date_collected` <= %s " % (route, i, lower_date_length, i, upper_date_length, proj_length_pair[1], proj_length_pair[0], depart_weekday, return_weekday, inp['date_collected']))
                query_ex = self.cur.execute(query)
                table = self.cur.fetchall()
                if len(table) > 0:
                    columns = [x for x in table[0].keys()]
                    ...
                    for index_2, j in enumerate(historical_windows):
                        ...
                        group = temp_rows[(
                                    ...
                                    )][:,(columns.index('rel_inc'),columns.index('abs_inc'),columns.index('count'))]

                        if group[:,0].size > 0:
                            avg_rel =  numpy.mean(group[:,0])
                            avg_abs =  numpy.mean(group[:,1])
                            avg_count =  numpy.mean(group[:,2])
                        else:
                            avg_rel = None
                            avg_abs = None
                            avg_count = None
                        ...
                        if group[:,0].size > 1:
                            st_dev_rel = numpy.std(group[:,0])
                            st_dev_abs = numpy.std(group[:,1])
                            st_dev_count = numpy.std(group[:,2])
                        else:
                            st_dev_rel = None
                            st_dev_abs = None
                            st_dev_count = None

                        row['%s_length_avg_rel_change_%s' % (i,j)] = avg_rel
                        row['%s_length_st_dev_rel_change_%s' % (i,j)] = st_dev_rel
                        row['%s_length_avg_abs_change_%s' % (i,j)] = avg_abs
                        row['%s_length_st_dev_abs_change_%s' % (i,j)] = st_dev_abs
                        #row['%s_length_avg_change_%s_count' % (i,j)] = avg_count
                        #row['%s_length_st_dev_change_%s_count' % (i,j)] = st_dev_count
                else:
                    for index_2, j in enumerate(historical_windows):
                        row['%s_length_avg_rel_change_%s' % (i,j)] = None
                        row['%s_length_st_dev_rel_change_%s' % (i,j)] = None
                        row['%s_length_avg_abs_change_%s' % (i,j)] = None
                        row['%s_length_st_dev_abs_change_%s' % (i,j)] = None
                        #row['%s_length_avg_change_%s_count' % (i,j)] = None
                        #row['%s_length_st_dev_change_%s_count' % (i,j)] = None


            if counter == 0:
                if not self.does_table_exist('%s_hist_movement' % (source)):
                    columns = {}
                    for k, v in row.iteritems():
                        if k == 'date_collected' or k == 'date_completed' or k == 'depart_date' or k == 'return_date':
                            columns['%s' % (k)] = "DATE"
                        else:
                            columns['%s' % (k)] = "DOUBLE"
                    self.replace_existing_table('%s_hist_movement' % (source), columns)
            ...
            self.insert_data('%s_hist_movement' % (source), row)
    ...
    def recent_min_fares(self, route, departing, returning):

        query = ("SELECT min(`depart_length`), `depart_date`, `return_date`, `price` "
                "FROM `%s_min_fares` "
                "WHERE `depart_date` >= '%s' "
                "AND `depart_date` <= '%s' "
                "AND `return_date` >= '%s' "
                "AND `return_date` >= '%s' "
                "GROUP BY `depart_date`, `return_date` " % (route, departing[0], departing[1], returning[0], returning[1]))
        self.cur.execute(query)
...
class search_inputs(object):
    ...
    def __init__(self, origin = "SFO", destination = "MAD", flight_type = "rt", lockin_per = 2, source = '',
