...
class simulation(search_inputs):
    ...
    def __init__(self, inputs):
        ...
        if len(self.error.error_list) > 0:
            ...
            rows = self.find_rows()
            if not rows['dates'] and self.prefs.id != 0:
                while not rows['dates']:
            ...
            if not rows['dates']:
                self.error.error_list.append(rows['error'])
                self.output['error'] = self.error.describe()
                ...
                try:
                    ...
                    prev_date1 = rows['prev_date1']
                    prev_date2 = rows['prev_date2']
                    ...
                    current = current_max(self, self.first_week_combos, self.second_week_combos, prev_date2)
    ...
    def find_rows(self):
        """
        @summary: this serves three purposes:
            ...
            3) checks that potential travel dates are not on black list
        """
        ...
        prev_date1 = self.search_date(self.d_date1, date_bank, 0)
        prev_date2 = self.search_date(self.d_date2, date_bank, 0)
        if not prev_date1 or not prev_date2:
            return {'dates': None, 'error': 11}
        ...
        # check if dates selected are on black list
        def check_black_list(combos, beg_period, ret_dates):
            for i in combos.iterkeys():
                for j in ret_dates:
                    black_list_res = self.db_dict.sel_crit('one', '%sblack_list' % (self.source), ['*'], {'route': self.route, 'flight_type': self.flight_type, 'num_high_days': i, 'prefs': self.prefs.id, 'beg_period': beg_period, 'ret_beg_period': j})
                    if black_list_res:
                        return True

        try:
            for i in range(self.max_trip_length):
                date_bank.append(date_bank[-1] + datetime.timedelta(weeks = 1))

            ret_beg_dates = []
            ret_beg_dates.append(self.search_date(self.r_date1, date_bank, 0))
            ret_beg_date_2 = self.search_date(self.r_date2, date_bank, 0)
            if ret_beg_date_2 not in ret_beg_dates:
                ret_beg_dates.append(ret_beg_date_2)

            if check_black_list(self.first_week_combos, prev_date1, ret_beg_dates) or check_black_list(self.second_week_combos, prev_date2, ret_beg_dates):
                return {'dates': None, 'error': 16}
        except:
            pass

        return {'dates': rows, 'error': None, 'prev_date1': prev_date1, 'prev_date2': prev_date2,}
...
class current_max(search_inputs):
    ...
    def __init__(self, inputs, first_week_combos, second_week_combos, prev_date2):


        self.initialize(inputs)
        self.db = db(cursorclass=MySQLdb.cursors.DictCursor, db = 'steadyfa_projection_prep')
        self.max_fare = self.loop_potential_fare_types(first_week_combos, second_week_combos, prev_date2)
        self.db.db_disconnect()


    def loop_potential_fare_types(self, first_week_combos, second_week_combos, prev_date2):
        try:
            bank = []

            if prev_date2 > self.d_date1:
                week_one_end = prev_date2-datetime.timedelta(days = 1)
            else:
                week_one_end = self.d_date2

            for i in first_week_combos.iterkeys():
                query_res = self.db.find_simple_sum_stat('%s%s' % (self.origin, self.destination), 'MAX', 'min_fare', where={'prefs': self.prefs.id, 'num_high_days': i, 'depart_length': (self.lockin_per*7)}, greater={'depart_date': self.d_date1, 'depart_date,trip_length,+,date_inc': self.r_date1}, less={'depart_date': week_one_end, 'depart_date,trip_length,+,date_inc': self.r_date2}, db_dict=True)
                if query_res:
                    bank.append(query_res)

            if prev_date2 > self.d_date1:
                for i in second_week_combos.iterkeys():
                    query_res = self.db.find_simple_sum_stat('%s%s' % (self.origin, self.destination), 'MAX', 'min_fare', where={'prefs': self.prefs.id, 'num_high_days': i, 'depart_length': (self.lockin_per*7)}, greater={'depart_date': prev_date2, 'depart_date,trip_length,+,date_inc': self.r_date1}, less={'depart_date': self.d_date2, 'depart_date,trip_length,+,date_inc': self.r_date2}, db_dict=True)
                    if query_res:
                        bank.append(query_res)
...
if __name__ == "__main__":
    ...
    inputs = search_inputs(purpose='simulation', start_date=datetime.date(2013,3,19))
