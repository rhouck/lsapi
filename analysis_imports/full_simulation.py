...
class full_simulation(object):
    ...
    def __init__(self, source_ext = '', num_proj_groups = 1, options_per_group = 40, start_date_days_back = 150, start_date_width = 100,
                 valid_attempts = 1, max_hold_per = 10, no_prefs = True, single_day_only = False, early_exercise = False, geography="us"):
        ...
        self.options_source = 'temp_options%s' % (source_ext)
        self.accuracy_testing_source = 'temp_accuracy_testing%s' % (source_ext)
    ...
    def analyze_existing(self):
        if self.db.does_table_exist(self.options_source):
            self.summary = data_set_analysis(import_table(self.options_source, db_name='steadyfa_temp_tables'))

    def move_results_from_temp(self):
        self.db_model.copy_table_from_other_db(self.options_source, self.options_source, 'steadyfa_temp_tables')
        self.db_model.copy_table_from_other_db(self.accuracy_testing_source, self.accuracy_testing_source, 'steadyfa_temp_tables')
    ...
    def run(self, test_projections=False):
        ...
        self.db.replace_existing_table(self.options_source, columns)
        ...
        for i in range(self.num_proj_groups):
            ...
            origin = self.db_model.get_random_row('hubs_%s' % (sel_geo), column='Airport')
            destination = self.db_model.get_random_row('destinations_%s' % (sel_geo), column='Airport')
            ...
            while valid == False:
                ...
                try:
                    ...
                    self.max_proj_week = self.db.find_simple_sum_stat('temp_projections', 'Max', 'proj_week')
                ...
                for j in range(self.options_per_group):
                    ...
                    if valid:
                        ...
                        if 'output' not in results:
                            ...
                            exercise = exercise_option(results, self.options_source, self.db, self.early_exercise)
    ...
    def find_percentile(self, sample_size, trials, category, stat):
        ...
        if self.db.does_table_exist(self.options_source):
            ...
            for i in range(trials):
                print 'trial: %s' % (i)
                trial = data_set_analysis(import_table(self.options_source, db_name='steadyfa_temp_tables'))
...
if __name__ == "__main__":
    ...
    example.run(test_projections=False)
    ...
    example.move_results_from_temp()

    """
    example = full_simulation(source_ext)
