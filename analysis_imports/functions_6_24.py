...
class db(object):
    ...
    def sel_crit(self, amt, table, what, where):
    ...
    def pare_down(self, table, max_date_collected, min_date_collected):

        self.drop_existing('pared')

        if isinstance(min_date_collected, int):
            min_date_collected = max_date_collected - datetime.timedelta(weeks = min_date_collected)
        if not min_date_collected:
            min_date_collected = datetime.date(2000,1,1)
        query = ("CREATE TABLE pared "
                "SELECT * FROM %s WHERE (`date_collected` >= '%s' AND `date_collected` <= '%s') "
                "ORDER BY `date_collected` DESC " % (table, min_date_collected, max_date_collected))
        query_ex = self.cur.execute(query)


    def build_table_a(self, route, max_date_collected=None, min_date_collected=None):
        # update these queries to allow for alternate routes and prefs

        self.drop_existing('a')
    ...
    def group_weekdays(self,route, min_dep_date=None):
        ...
        self.build_table_a(route, min_dep_date=min_dep_date)
        ...
        query = ("CREATE TABLE labels "
                ...
                "FROM `a` " % (avg, st_dev, avg, st_dev,avg, st_dev, avg, st_dev,))
    ...
    def join_fares(self, route, max_dist=None, flex=True, max_date_collected=None, min_date_collected=None):
        ...
        query = ("CREATE TABLE %s_price_change_source "
                ...
                "ORDER BY a.date_collected DESC " % (route))

        """
        query = ("CREATE TABLE %s_price_change_source "
                "SELECT a.price as beg_price, b.price as end_price, a.count, a.date_collected, a.trip_length, "
                "a.depart_date, a.return_date, a.dep_wk_day, a.ret_wk_day, a.day_tag_d, a.day_tag_r, "
                "a.depart_length as depart_length, (a.depart_length + a.trip_length) as return_length, "
                "b.price-a.price as abs_inc,  b.price/a.price as rel_inc "
                "FROM a, a as b "
                "WHERE abs(b.depart_length - IF(a.depart_length<%s,0,a.depart_length-%s)) < %s "
                "AND a.depart_length > b.depart_length "
                "AND abs(a.depart_date - b.depart_date) <= %s "
                "AND abs(a.return_date - b.return_date) <= %s "
                "AND a.dep_wk_day = b.dep_wk_day "
                "AND a.ret_wk_day = b.ret_wk_day " % (route, max_dist, max_dist, dep_len_window, date_window, date_window))
        """
        query_ex = self.cur.execute(query)
        self.drop_existing('a')
    ...
    def find_hist_analysis(self, route, unique_inputs, historical_windows):
        ...
        table = self.import_table('%s_price_change_source_short' % (route))
        columns = [x for x in table[0].keys()]
        rows = numpy.asarray([[x for x in row.values()] for row in table])
        ...
        for index, i in enumerate(('depart','return')):
            ...
            for counter, inp in enumerate(unique_inputs['price_%s_date' % (i)]):
                ...
                #second_date_collected = inp['date_collected'] + datetime.timedelta(weeks = inp['proj_length'])
                row = dict(inp.items())
                ...
                for index_2, j in enumerate(historical_windows):
                    ...
                    group = rows[(
                                (rows[:,columns.index('%s_date' % (i))]>=lower_date) &
                                (rows[:,columns.index('%s_date' % (i))]<=upper_date) &
                                (rows[:,columns.index('%s_length' % (i))]>=lower_date_length) &
                                (rows[:,columns.index('%s_length' % (i))]<=upper_date_length) &
                                (rows[:,columns.index('day_tag_d')]==depart_weekday) &
                                (rows[:,columns.index('day_tag_r')]==return_weekday)
            ...
            for counter, inp in enumerate(unique_inputs['inc_%s_date' % (i)]):
                ...
                for index_2, j in enumerate(historical_windows):
                    ...
                    group = rows[(
                                (rows[:,columns.index('date_collected')]<=lower_date) &
                                (rows[:,columns.index('date_collected')]>=upper_date) &
                                (rows[:,columns.index('%s_length' % (i))]>=lower_date_length) &
                                (rows[:,columns.index('%s_length' % (i))]<=upper_date_length) &
                                (rows[:,columns.index('proj_length')]>=proj_length_pair[1]) &
                                (rows[:,columns.index('proj_length')]<=proj_length_pair[0]) &
                                (rows[:,columns.index('day_tag_d')]==depart_weekday) &
                                (rows[:,columns.index('day_tag_r')]==return_weekday)
            ...
            master["%s_inc" % (i)] = row_bank

        # remove pickling once data is saved in database
        file_object = open("%s\model_sessions\%s_hist_movement_analysis" % (os.getcwd(),route),'w')
        pickle.dump(master, file_object)
        file_object.close()
        return master

    def impute_missing(self, route, proj_date):

        # remove pickling once data is retrieved from database

        file_object = open("%s\model_sessions\%s_hist_movement_analysis" % (os.getcwd(),route),'r')
        master = pickle.load(file_object)
        for index, i in enumerate(master.iterkeys()):
            if index == 0:
                temp_array = copy.deepcopy(master[i])
                labels = temp_array[0].keys()
                labels = [i for i in labels if not isinstance(temp_array[0][i],datetime.date)]

                # delete items if date is greater than the projection date
                temp_array = [i for i in temp_array if i['date_collected'] <= proj_date]

                # convert dictionaries to list and delete date items
                temp_array = numpy.asarray([[j for j in i.itervalues() if not isinstance(j,datetime.date)] for i in temp_array])
                non_zero = [numpy.count_nonzero(temp_array[:,i]) for i in range(temp_array.shape[1])]
                count = temp_array.shape[0]

                print non_zero
                print numpy.true_divide(non_zero,count)
    ...
    def pop_inputs(self, route, historical_windows):
        ...
        unique_inputs = {}
        query = ("SELECT DISTINCT `date_collected`, `depart_date`, `day_tag_r`  FROM `%s_price_change_source_short` " % (route))
        query_ex = self.cur.execute(query)
        unique_inputs['price_depart_date'] = self.cur.fetchall()

        query = ("SELECT DISTINCT `date_collected`, `return_date`, `day_tag_d`  FROM `%s_price_change_source_short` " % (route))
        query_ex = self.cur.execute(query)
        unique_inputs['price_return_date'] = self.cur.fetchall()

        query = ("SELECT DISTINCT `date_collected`, `depart_date`, `day_tag_r`, `proj_length`  FROM `%s_price_change_source_short` " % (route))
        query_ex = self.cur.execute(query)
        unique_inputs['inc_depart_date'] = self.cur.fetchall()

        query = ("SELECT DISTINCT `date_collected`, `return_date`, `day_tag_d`, `proj_length`  FROM `%s_price_change_source_short` " % (route))
        query_ex = self.cur.execute(query)
        unique_inputs['inc_return_date'] = self.cur.fetchall()

        hist_analysis_bank = self.find_hist_analysis(route, unique_inputs, historical_windows)
        #for row in hist_analysis_bank:
        #    self.insert_data('%s_hist_movement_analysis' % (route), row)


    """
    def pop_inputs(self, route, start_date, dep_date_week):

    flight_type = 'rt'
    historical_windows = (1,2,4,8)

    self.drop_existing('price_change_with_day_ids')
    self.drop_existing('price_change_with_cats')
    columns = {'travel_dates_cat': 'INT(3)', 'depart_length_week': 'INT(3)', 'return_length_week': 'INT(3)'}
    for i in ('depart','return'):
        columns['%s_date_range_beg' % (i)] = "DATE"
        columns['%s_date_range_end' % (i)] = "DATE"
        for j in historical_windows:
            columns['%s_date_avg_change_%s' % (i,j)] = "DOUBLE"
            columns['%s_date_st_dev_return_%s' % (i,j)] = "DOUBLE"
            columns['%s_length_avg_change_%s' % (i,j)] = "DOUBLE"
            columns['%s_length_st_dev_return_%s' % (i,j)] = "DOUBLE"
    self.replace_existing_table('hist_movement_analysis', columns)


    id_pair_tags = weekday_pairs(flight_type)
    query_inserts = id_pair_tags.tag_query_values()

    query = ("CREATE TABLE price_change_with_day_ids SELECT *, %s, %s "
             "FROM `%s_price_change_source`" % (query_inserts['depart_tag'],query_inserts['return_tag'], route))
    query_ex = self.cur.execute(query)
    query = ("CREATE TABLE price_change_with_cats SELECT *, %s "
             "FROM `price_change_with_day_ids`" % (query_inserts['convert_tags']))
    query_ex = self.cur.execute(query)


    # get latest depart date
    query = ("SELECT max(`depart_date`) FROM `price_change_with_cats`")
    query_ex = self.cur.execute(query)
    latest_dep_date = self.cur.fetchone()[0]
    iter_weeks = int(math.floor((latest_dep_date - start_date).days/7.0))

    # get longest trip length
    query = ("SELECT max(`trip_length`) FROM `price_change_with_cats`")
    query_ex = self.cur.execute(query)
    longest_trip_length = self.cur.fetchone()[0]
    ret_date_week = int(math.floor(longest_trip_length/7.0))

    for iter_week in range(iter_weeks):
        start_date_adjust = start_date + datetime.timedelta(days = (iter_week*7))
        row = {}
        for d in range(0,dep_date_week+1):
            row['depart_length_week'] = d
            for r in range(0,ret_date_week+1):
                row['return_length_week'] = d + r
                dep_date_week_date = start_date_adjust
                ret_date_week_date = dep_date_week_date + datetime.timedelta(days = (r*7))
                row['depart_date_range_beg'] = "'%s'" % (dep_date_week_date)
                row['depart_date_range_end'] = "'%s'" % (dep_date_week_date + datetime.timedelta(days = 6))
                row['return_date_range_beg'] = "'%s'" % (ret_date_week_date)
                row['return_date_range_end'] = "'%s'" % (ret_date_week_date + datetime.timedelta(days = 6))

                print "Iter: %s -- (count: %s) depart: %s -- return: %s -- depart length: %s, return length: %s" % (iter_week, (d+1)*(ret_date_week+1)+r-1, dep_date_week_date, ret_date_week_date, d, d+r)

                for c in id_pair_tags.pair_ids.iterkeys():
                    row['travel_dates_cat'] = c
                    for i in historical_windows:
                        for j, end_date, travel_length in (('depart', dep_date_week_date, d), ('return', ret_date_week_date, d+r)):
                            beg_date = end_date - datetime.timedelta(days = (i*7))
                            end_trav_length = travel_length * 7
                            beg_trav_length = (travel_length + 1) * 7

                            # filter by date
                            query = ("SELECT avg(`abs_inc`), stddev(`rel_inc`) "
                                    "FROM `price_change_with_cats` "
                                    "WHERE (`%s_date` >= '%s' AND `%s_date` < '%s') "
                                    "AND (`%s_length` >= %s AND `%s_length` < %s) "
                                    "AND `travel_dates_cat` = %s " % (j, beg_date, j, end_date, j, end_trav_length, j, beg_trav_length, c))
                            #print query
                            query_ex = self.cur.execute(query)
                            avg_change, st_dev_return = self.cur.fetchone()
                            row['%s_date_avg_change_%s' % (j,i)] = avg_change if avg_change else "NULL"
                            row['%s_date_st_dev_return_%s' % (j,i)] = st_dev_return if st_dev_return else "NULL"

                            beg_date = end_date - datetime.timedelta(days = (7))
                            end_trav_length = travel_length * 7
                            beg_trav_length = (travel_length + i) * 7

                            # filter by time to travel
                            query = ("SELECT avg(`abs_inc`), stddev(`rel_inc`) "
                                    "FROM `price_change_with_cats` "
                                    "WHERE (`%s_date` >= '%s' AND `%s_date` < '%s') "
                                    "AND (`%s_length` >= %s AND `%s_length` < %s) "
                            ...
                            query_ex = self.cur.execute(query)
                            avg_change, st_dev_return = self.cur.fetchone()
                            row['%s_length_avg_change_%s' % (j,i)] = avg_change if avg_change else "NULL"
                            row['%s_length_st_dev_return_%s' % (j,i)] = st_dev_return if st_dev_return else "NULL"

                    self.insert_data('hist_movement_analysis', row)
    ...
    def join_fares_on_hist_analysis(self, route):
        ...
        hist_move = list(self.import_table('%s_hist_movement_analysis' % (route)))
        ...
        for index, i in enumerate(price_changes):
            ...
            if len(hist_move) == 0:
                ...
                for index_2, j in enumerate(hist_move):
                    #print "histmove index: %s " % (index_2)
                    if i['depart_date'] == j['depart_date'] and i['return_date'] == j['return_date'] and i['date_collected'] == j['date_collected'] and i['proj_length'] == j['proj_length']:
                        i['date_collected'] = "'%s'" % (i['date_collected'] + datetime.timedelta(days = i['proj_length']))
                        del i['depart_date']
                        del i['return_date']
                        del i['abs_inc']
                        del i['beg_price']
                        ...
                        del j['proj_length']
                        del j['id']
                        new_table.append(dict(i.items() + j.items()))
                        print "matched index: %s" % (index_2)
                        del hist_move[index_2]
                        break
