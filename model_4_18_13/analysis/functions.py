...
class db(object):
    # user = "steadyfa_model", passwd = "buttwatercruises", db = "steadyfa_sf_model"
    # user = "root", passwd = "bitnami", db = "api_projection_model"
    def __init__(self, host = "localhost", user = "steadyfa_model", passwd = "buttwatercruises", db = "steadyfa_sf_model", cursorclass = None, unix_socket = '/opt/bitnami/mysql/tmp/mysql.sock'):
        self.host = host
        self.user = user 
        self.passwd = passwd
        self.db = db
...
class sim_errors(object): 
    ...
    def __init__(self, db, origin, destination, flight_type, lockin_per, start_date, d_date1, d_date2, r_date1, r_date2, final_proj_week, max_trip_length, geography):   
        ...
        self.error_desc[11] = 'Sorry, there was insufficient data to calculate your steadyfare. Your search was re-run with more relaxed criteria.'
        self.error_desc[12] = 'Sorry, there was insufficient data to calculate your steadyfare. Please try another search with more relaxed criteria.'
...
class weekday_pairs(object):
    ...
    def tag_query_values(self):
        ...
        depart_tag = "If(day_tag_d=0,%s,If(day_tag_d=1,%s,If(day_tag_d=2,%s,If(day_tag_d=3,%s,If(day_tag_d=4,%s,If(day_tag_d=5,%s,If(day_tag_d=6,%s,NULL))))))) as day_tag_d " % (weekday_ids[0], weekday_ids[1], weekday_ids[2], weekday_ids[3], weekday_ids[4], weekday_ids[5], weekday_ids[6])
        return_tag = "If(day_tag_r=0,%s,If(day_tag_r=1,%s,If(day_tag_r=2,%s,If(day_tag_r=3,%s,If(day_tag_r=4,%s,If(day_tag_r=5,%s,If(day_tag_r=6,%s,NULL))))))) as day_tag_r " % (weekday_ids[0], weekday_ids[1], weekday_ids[2], weekday_ids[3], weekday_ids[4], weekday_ids[5], weekday_ids[6])
        ...
        for k, v, in convert_ids.iteritems():
            if self.order_matters:
                 convert_tags += "If(day_tag_d + (day_tag_r * 10)=%s,%s," % (k, v)
            else:
                convert_tags += "If(day_tag_d + day_tag_r=%s,%s," % (k, v)
            end_string += ')'
        convert_tags += end_string + ' as num_high_days '
...
class search_inputs(object):
    ...
    def __init__(self, origin = "SFO", destination = "JFK", flight_type = "rt", lockin_per = 2, source = '',  
                 ...
                 d_date1 = datetime.timedelta(days = 40), d_date2 = datetime.timedelta(days = 0), 
                 r_date1 = datetime.timedelta(days = 51), r_date2 = datetime.timedelta(days = 0), 
                 cycles = 1000, wtp = 0, wtpx = 0, buffer = 7.5, correl_coef = 1, round_to = 1, 
                 # projection specific assumptions
                 num_wks_proj_out = 20, final_proj_week = 1, first_proj_week = 20, num_per_look_back = 5, depart_length_width = 1, width_of_avg = 1, # width_of_avg greater than 1 relaxes searches for current price as well as minimum summary stats for finding changes in value  
                 num_high_days = 1, min_change = None, weight_on_imp = 1, ensure_suf_data = .5, regressed = False, seasonality_adjust = True, geography = "us",
                 ):
        ...
        if 'projection' in purpose:
            ...
            self.end_period = self.start_date + datetime.timedelta(days = 0)
            self.beg_period = self.end_period + datetime.timedelta(days = -7) 
            self.type = type
