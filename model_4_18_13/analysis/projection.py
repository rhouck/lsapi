...
class projection(search_inputs):
    ...
    def __init__(self, inputs):
        ...
        try:
            self.db_mins.pull_mins(self.origin, self.destination, self.earliest_source_date, self.start_date, self.flight_type, self.max_trip_length, self.prefs.id, self.num_high_days)                          
        ...
        #print "pulled mins in " + `time.time()-start` + " seconds"
        ...
        #print "average current fares in " + `time.time()-start` + " seconds"
        
        # apply rates of change, etc, to generate projection set
        self.project()    
        #print "projected in " + `time.time()-start` + " seconds"
...
class current_fares(projection):
    ...
    def projection(self):
        ...
        #self.db_current = db(db='steadyfa_projection_prep')
        ...
        for i in range(self.final_proj_week, self.num_wks_proj_out + 1):
            ...
            #avg_fare = self.db_current.find_min_sum_stat('Avg', self.origin, self.destination, self.num_high_days, self.prefs.id, depart_dates, depart_lengths)
...
class wkly_fare_movement(projection):
    ...
    def run(self):
        try:
            if self.type == "implied" and self.seasonality_adjust:
                ...
                change_adjustment_bank = dict()        
                for i in range(self.final_proj_week,(self.first_proj_week+1)):
                    for k in range((i), (self.first_proj_week+2)):
                        try:
                            # find general implied trend
                            if i in change_adjustment_bank:
                                if k not in change_adjustment_bank[i]:
                                    change_adjustment_bank[i][k] = calc_adjustment(k)
                                
                            else:
                                change_adjustment_bank[i] = dict()
                                change_adjustment_bank[i][k] = calc_adjustment(k)
                        except:
                            change_adjustment_bank[i][k] = None
                ...
                for j in range(self.num_per_look_back):
                    for k in range((i+1), (self.first_proj_week+2)):
                        ...
                        if self.type == "implied" and self.seasonality_adjust:
                            ...
                            try:
                                implied_change_adjustment = (sum(short_bank)/len(short_bank)) # **(1/(self.num_per_look_back-1))
                            except:
    ...
    def truncate_empty_data_set(self, set):
        ...
        for i in new_set.iterkeys():
            # this prevents adding keys after long gaps in which there was no data, which would prevent projections from generating any results
    ...
    def stats_on_mins(self, proj_week, look_back_per, trip_length=None):        
        ...
        #avg_fare = self.db_movement.find_min_sum_stat('Avg', self.origin, self.destination, self.num_high_days, self.prefs.id, depart_dates, depart_lengths)
        avg_fare = self.db_movement.find_min_sum_stat('Avg', depart_dates, depart_lengths)
        ...
        #return {'avg_fare': avg_fare, 'adj_beg_period': adj_beg_period, 'adj_end_period': adj_end_period, 'proj_week': proj_week}
...
class projection_set(object):
    ...
    def __init__(self, inputs=None, record=False, update_projection_prep=False):
    ...
    def setup_temp_tables(self):
        ...
        def build_columns(self, type):
            ...
            for i in range(self.inputs.final_proj_week, max(num_columns) + 1):
            """
...
if __name__ == "__main__":
    ...
    inputs = search_inputs(purpose='projection', geography="us", start_date=datetime.date(2013,3,19))
    ...
    inputs = search_inputs(purpose='projection', start_date=datetime.date(2013,3,19)) 
    example = projection_set(inputs, record=False, update_projection_prep=False)
    example.loop_num_high_days()
    pprint(example.set) 
